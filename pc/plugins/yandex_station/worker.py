...
class DeviceWorker:
    ...
    def start(self):
        ...
        self.plugin.loops[self.device_id] = self.loop
        
        async def run():
            while not self.plugin._stop_event.is_set():
                try:
                    ...
                    self.log(f"Connection error for {self.device_id}: {e}", level="error")
                    self.plugin.states[self.device_id]["online"] = False
                    self.plugin._push_state()
                    
                    if "4000" in str(e) or "Invalid token" in str(e):
                        if self.plugin.auth.refresh_tokens_sync():
                            await asyncio.sleep(2)
                            continue
                    await asyncio.sleep(5)
        
        self.log(f"Worker for {self.device_id} is starting its loop...")
        try:
            self.loop.run_until_complete(run())
        except (RuntimeError, asyncio.CancelledError):
            pass 
        finally:
            self.log(f"Worker for {self.device_id} has stopped.")
