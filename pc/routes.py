...
def register_routes(app, p_manager):
    ...
    def karaoke_static(device_id):
        ...
        # 2. Создание изображения (копируем логику из потока)
        img = Image.new('RGB', (1280, 720), color=(10, 10, 15))
        try:
            # Шрифты
            font_path = "C:\\Windows\\Fonts\\arial.ttf"
            font_path_bold = "C:\\Windows\\Fonts\\arialbd.ttf"
            f_main = ImageFont.truetype(font_path_bold, 50)
            f_sub = ImageFont.truetype(font_path, 35)
            f_meta = ImageFont.truetype(font_path_bold, 24)
            f_artist = ImageFont.truetype(font_path, 20)
        except:
    ...
    def karaoke_stream(device_id):
        def generate():
            ...
            while True:
                ...
                if timings:
                    # Режим караоке: вычисляем текущую строку
                    now = time.time()
                    last_upd = track_info.get('last_upd', now)
                    progress = track_info.get('progress', 0)
                    
                    # Интерполяция времени (прогресс + время с последнего обновления)
                    current_time_ms = progress * 1000
                    if track_info.get('playing'):
                        current_time_ms += (now - last_upd) * 1000
                    
                    # Ищем текущую и следующую строки
                    for i, t_line in enumerate(timings):
                        if t_line['time'] <= current_time_ms:
                            ...
                            # Если мы еще не дошли до первой строки
                            if i == 0:
                                secondary_text = t_line['text']
                            break
                elif full_lyrics:
                ...
                # Рисуем текущую строку (Акцентный цвет)
                draw_text_centered(display_text, 300, font_main, (56, 189, 248, 255))
                # Рисуем следующую строку (Тусклый цвет)
                draw_text_centered(secondary_text, 420, font_sub, (200, 200, 200, 150))
